...
bool UART_PM_Sensors::GetParticleCount( float* value, UART_PM_Sensors::ParticleSize_t ps ){
  ...
  float result = 0;
    ...
    //We need to copy the value from our sensor to the pointer
    *value = result;
    return reading_ok;
...
float UART_PM_Sensors::GetValue( DATAUNITS::MessurmentValueType_t Type, uint8_t channel ){
    ...
    if(DATAUNITS::MessurmentValueType_t::PARTICLES != Type){
        value = NAN;
        Serial.println("UART PM Type != DATAUNITS::MessurmentValueType_t::PARTICLES");
    } else {
            switch(channel){
                case 0:{
                    //PM2.5
                    if(true == GetParticleCount(&value, UART_PM_Sensors::ParticleSize_t::PM2_5) ){
                        ...
                        value = NAN;
                        Serial.println("UART PM Timeout");
                    }
                } break;
                ...
                default:{
                    value = NAN;
                    Serial.println("UART PM unknown channel");
                } break;
            }
    }
