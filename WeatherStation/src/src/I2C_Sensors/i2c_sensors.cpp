    ...
    switch( channelidx ){
        ...
        case 0:{
            if(true == DeviceOnBus[TSL2561]){
                    ...
                    if(true == tsl2561.getEvent(&event) ){
                        if ((event.light == 0) ||
                        (event.light > 4294966000.0) || 
                        (event.light <-4294966000.0))
                        {
                            Serial.printf("TSL2561 light out of range: %f", event.light);
                            value = NAN;
                        } else {
                            value = event.light;
                        }    
                    } else {
                         Serial.println("TSL2591 read event failed");
                    }              
            } else {
                value = NAN;
            }
        ...
        case 1:{
            if(true == DeviceOnBus[TSL2591]){
                 sensors_event_t event;
                if(true == tsl2591.getEvent(&event)){
                  if ( (event.light == 0) ||
                       (event.light > 4294966000.0) || 
                       (event.light <-4294966000.0))
                    {
                        Serial.printf("TSL2591 light out of range: %f", event.light);
                        value = NAN;
                    } else {
                        value = event.light;
                    }        
        ...
        default:{
            Serial.printf("Light channel %u not existing\n\r", channelidx );
        } break;
    } 
